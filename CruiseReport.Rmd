...
```{r include=FALSE}
# Planned track line
orig <- read.csv('SE2204_wpt_timing_140.csv')
# Argo drop locations
argo <- data.frame(lon=c(-155.95, -153.61, -151.7, -150, -150, -150, -(155+(52.35/60)), -(153+(22.81/60)), -(151+(44.2/60)), -(149+(46.2/60)), -(149+(50.29/60)), -(149+(56.3/60))), lat=c(23.5, 26, 28, 27, 24, 21, (23+(14.85/60)), (26+(14.35/60)), (27+(57.71/60)), (27+(21/60)), (24+(6.81/60)), (21+(8.7/60))), type=rep(c('planned', 'acctual'),each=6))
# CTD data
test <- read.csv('CTD/SE2204_CTDlocations.csv')
test$Date_HST <- as.POSIXct(test$Date_HST, format='%m/%d/%y %H:%M')
edna <- read.csv('eDNA_Metadata_BigeyeSE2204 - CTD Casts.csv')
test <- test %>% mutate(Cast=1:nrow(test)) %>% 
  left_join(edna[,c(8,14)], by='Cast')
  
# Staff
staff <- read.csv('SE2204_participants_2.csv')
# net tow logs
bongo <- read.csv('SE2204_bongo_log.csv')
mocness <- readRDS('SE2204_MOCNESS_jwEdit.rds')
ring <- read.csv('SE2204_2mRingNet_log_08222022_B.csv') %>% na.omit()
ikmt <- read.csv('SE2204_IKMT_log.csv')

ring$start_date <- as.POSIXct(ring$start_date, format='%m/%d/%y')
ring$end_date <- as.POSIXct(ring$end_date, format='%m/%d/%y')
ikmt$start_date <- as.POSIXct(ikmt$start_date, format='%m/%d/%y')
ikmt$end_date <- as.POSIXct(ikmt$end_date, format='%m/%d/%y')
```
...
files = dir(path = "CTD/", pattern = "downOnly_loopEdit.cnv", full.names = TRUE )
...
test <- data.frame(ID=1:length(section@metadata$time), Station=files, Date=section@metadata$time, Lon=section@metadata$longitude, Lat=section@metadata$latitude)
# Make time in HST
test$Date_HST <- with_tz(test$Date, 'HST')
test$Time <- strftime(test$Date_HST, format='%H%M')
# Simplify station names
idx <- strsplit(test$Station, '_downOnly_loopEdit.cnv')
idx <- unlist(strsplit(unlist(idx), './SE-22-04_'))
...
test <- test %>% 
  select(Station, Date_HST, Time, Lon, Lat, Depth, Date) %>% 
  arrange(Date_HST)
```
...
NOAA Ship *Oscar Elton Sette*, Project SE-22-04

**PROJECT**  
**PERIOD:** \tabto{1.5in} 
June 9 -- 12, 2022 gear trials

\tabto{1.5in} June 13 -- July 10, 2022 planned project start date

\tabto{1.5in} June 20 -- July 10, 2022 actual project start date

**AREA OF**  
**OPERATION:** \tabto{1.5in}
Operations were conducted in the waters of the North Pacific along the 

\tabto{1.5in} 150$^\circ$W meridian from 30$^\circ$N to 10.7$^\circ$N (Fig. 1).

**TYPE OF**  
**OPERATION:** \tabto{1.5in}
Conductivity-temperature-depth (CTD) casts to 1300 m depth were
...
s <- kbl(staff, booktabs=T, label=NA) %>%
  ...
  #kable_paper('striped', , font_size=10) %>%
  pack_rows("SE-22-04 Participants", 1, 15) %>%
  pack_rows("Gear Trial Participants", 16, 19) %>% 
  add_footnote(c('Pacific Islands Fisheries Science Center', 'National Marine Fisheries Service', 'University of Hawaii', 'Cooperative Institute for Marine and Atmospheric Reserach', 'University of South Carolina'), notation='number')

asis_output(str_replace_all(s, c("PIFSC1"="PIFSC$^{1}$", "NMFS2"="NMFS$^{2}$", "UH3"="UH$^{3}$", "CIMAR4"="CIMAR$^{4}$", "SC5"="SC$^{5}$")))
```
...
test$Station2[4] <- paste0('A night', 'a')
test$Station2[6] <- paste0('A day', 'b')
test$Station2[7] <- paste0('A day', 'c')
test$Station2[8] <- paste0(1, 'd')

t1 <- test %>% 
  mutate(Cast=1:nrow(test), Lon=Lon*-1) %>% 
  select(Cast, Station2, Date_HST, Lon, Lat, Depth) %>% 
  rename(`Date & Time (HST)` = Date_HST, `Latitude (°N)`=Lat, `Longitude (°W)`=Lon, Station=Station2, `Depth (m)`=Depth) %>% 
  kbl(booktabs=T, label='CTD operations', caption = 'CTD operations during SE-22-04.', longtable=T) %>%
  kable_styling(latex_options = c("striped", "HOLD_position", "repeat_header"), font_size=12) %>% 
  footnote(alphabet=c('Bottle #12 did not fire', 'Bottle #7 did not fire', 'Bottle #7 did not fire', 'Bottle #10 did not fire'))

asis_output(str_replace_all(t1, c("1d"="1$^{d}$", "nighta"="night$^{a}$", "dayb"="day$^{b}$", "dayc"="day$^{c}$")))
```
...
files <- files[c(1:13,15,21,27,33,39)]

# Start loop
for (i in 1:length(files)){
  ctdAll[[i]]  = read.ctd(files[i]) %>%
    ctdDecimate(p = 1) # align to the same standard pressure
}

# Draw profiles at multiple stations ctd
dat <- data.frame()
for (i in seq_along(ctdAll)) {
  dat <- ctdAll[[i]]@data %>% 
    as_tibble() %>% 
    mutate(Station=ctdAll[[i]]@metadata$header[19], Station=substring(Station, 11)) %>% 
    add_row(dat)
}
dat[which(dat$Station == '06' & dat$pressure <= 3),4:8] <- NA
...
vars <- c('temperature', 'salinity', 'sigmaTheta', 'oxygen', 'fluorescence')
varsUnit <- c('Temperature [C]', 'Salinity', 'Sigma Theta [kg/m3]', 'Dissolved Oxygen [mL/L]', 'Total Chlorophyll [mg/m3]')
